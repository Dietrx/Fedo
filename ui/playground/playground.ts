...
function build() {
  ...
  overlay = createOverlay({ theme, minScore: Number(minSel.value), stats: false, layout: layoutSel.value as Layout, dashboardUrl: "http://localhost:8766/dashboard.html", calmMode: (document.getElementById("calm") as HTMLInputElement).checked });
...
function simulateLive() {
  ...
  const timer = setInterval(() => {
    ...
    const progress = { phase: f < 1 ? ("transcribing" as const) : ("done" as const), coveredSec: Math.round(durationSec * f), durationSec, etaAt: f < 1 ? Date.now() + (8 - step) * 900 : undefined };
...
document.getElementById("calm")!.addEventListener("change", build);
