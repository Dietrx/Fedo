...
import { GLYPH, GROUPS, groupOf, isTopic, resolveTheme, themeCss, type AnyGroup } from "../theme";
...
function renderReadouts(all: LogEntry[]) {
  const withSig = all.filter((e) => e.signals.some((s) => s.score >= 0.5 && !isTopic(s.key)));
...
function renderTechniques(all: LogEntry[]) {
  ...
  for (const e of all) for (const s of e.signals) if (s.score >= 0.5 && !isTopic(s.key)) { count.set(s.key, (count.get(s.key) ?? 0) + 1); byGroup[groupOf(s.key)]++; }
...
function renderSources(all: LogEntry[]) {
  ...
  for (const e of all) {
    if (!e.signals.some((s) => s.score >= 0.5 && !isTopic(s.key))) continue;
    const cur = by.get(e.handle) ?? { n: 0, groups: new Set<AnyGroup>(), name: e.displayName, platform: e.platform };
    cur.n++;
    for (const s of e.signals) if (s.score >= 0.5 && !isTopic(s.key)) cur.groups.add(groupOf(s.key));
    by.set(e.handle, cur);
  }
...
function renderPosts(list: LogEntry[]) {
  ...
  $("posts").innerHTML = list.slice(0, limit).map((e) => {
    const shown = e.signals.filter((s) => s.score >= 0.5 && !isTopic(s.key));
    const topic = e.signals.filter((s) => s.score >= 0.5 && isTopic(s.key)).map((s) => `<span class="topic">${esc(SIGNALS[s.key].label.replace(/ content$/i, ""))} topic</span>`).join("");
    const labs = topic + shown.slice(0, 3).map((s) => `<span data-group="${groupOf(s.key)}">${mark(groupOf(s.key))}${esc(SIGNALS[s.key].label)} <b>${pct(s.score)}</b></span>`).join(`<span class="sep">·</span>`) + (shown.length > 3 ? `<b class="sep" style="color:var(--ink-muted)"> +${shown.length - 3}</b>` : "");
    ...
    // same ranking as the card: strongest first, below 50% shown quietly (the topic is a tag above, not a bar)
    const rows = e.signals.filter((s) => !isTopic(s.key)).map((s, i) => `<div class="row${s.score < 0.5 ? " weak" : ""}" data-group="${groupOf(s.key)}"><span class="rank">${i + 1}</span><span class="lbl">${mark(groupOf(s.key))}${esc(SIGNALS[s.key].label)}</span><span class="n">${pct(s.score)}</span><div class="meter"><span class="fill" style="width:${pct(s.score)}"></span></div>${s.evidence ? `<div class="ev">“${esc(s.evidence)}”</div>` : ""}</div>`).join("");
...
function demo(): LogEntry[] {
  const pool: Array<Partial<LogEntry> & { text: string; handle: string; sig: [SignalKey, number, string?][] }> = [
    ...
    { handle: "jane_bakes", displayName: "Jane", text: "Finally nailed the sourdough after 3 weeks. The trick was a longer cold proof overnight.", sig: [["political_persuasion", .02], ["fear_framing", .02], ["anger_framing", .02], ["us_vs_them", .02], ["scapegoating", .02]], overall: { level: "none", score: 0 } },
    { handle: "growth_guru_ai", displayName: "Growth Guru", text: "In today's fast-paced world, unlocking your potential is more important than ever. Here are 7 game-changing tips 🧵👇", sig: [["possible_ai_slop", .89, "In today's fast-paced world"], ["engagement_bait", .82], ["sensationalism", .35, "game-changing tips"], ["urgency_language", .2], ["commercial_persuasion", .12]], overall: { level: "medium", score: .5 }, slop: true, explanation: "This post shows patterns typical for mass-produced content and asks for engagement." },
...
onLogChange((l) => { log = l; render(); flash(); });
/** a short "Updated" pulse in the header when new analyses arrive while the page is open */
function flash() { const el = $("sync"); el.textContent = "Updated"; el.classList.add("on"); setTimeout(() => el.classList.remove("on"), 1500); }
